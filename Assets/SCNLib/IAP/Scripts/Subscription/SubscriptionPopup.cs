...
namespace SCN.IAP
{
    public class SubscriptionPopup : MonoBehaviour
    {
        ...
        private void Awake()
        {
            IAPManager.Instance.OnBuyDone += OnSubscription;
        }
        ...
        private void Start()
        {
            ...
            if (_buyButtonMonthly != null && _idSubscriptionMonthly != "")
            {
                ...
                {
                    var productMonthly = IAPManager.Instance.GetProduct(_idSubscriptionMonthly);
                    if(productMonthly != null)
                    FirebaseManager.instance.LogClickIAP("monthly_plan", productMonthly.metadata.localizedPriceString, productMonthly.metadata.isoCurrencyCode);
                    IAPManager.Instance.BuyProductID(_idSubscriptionMonthly, productMonthly.metadata.localizedTitle, _costBuyMonthlyTxt.text, productMonthly.metadata.isoCurrencyCode);
                });
            }
            if (_buyButtonAnnual != null && _idSubscriptionAnnual != "")
            {
                ...
                {
                    var product = IAPManager.Instance.GetProduct(_idSubscriptionAnnual);
                    if(product != null)
                    FirebaseManager.instance.LogClickIAP("annual_plan", product.metadata.localizedPriceString, product.metadata.isoCurrencyCode);
                    IAPManager.Instance.BuyProductID(_idSubscriptionAnnual, product.metadata.localizedTitle, _costBuyAnnualTxt.text, product.metadata.isoCurrencyCode);
                });
            }
            if (_buyButtonUnlockAll != null && _idUnlockAll != "")
            {
                ...
                {
                    var product = IAPManager.Instance.GetProduct(_idUnlockAll);
                    if(product != null)
                    FirebaseManager.instance.LogClickIAP("life_plan", product.metadata.localizedPriceString, product.metadata.isoCurrencyCode);
                    IAPManager.Instance.BuyProductID(_idUnlockAll, product.metadata.localizedTitle, _costBuyUnlockAllTxt.text, product.metadata.isoCurrencyCode);
                });
            }
