...
namespace SCN.IAP
{
    public class IAPManager : MonoBehaviour, IStoreListener, IDetailedStoreListener
    {
        ...
        public Action OnBuyDone = null, OnBuyFail = null;
        ...
        public void BuyProductID(string productId, string product_name, string price, string currency)
        {
            ...
            if (IsInitialized())
            {
                ...
                {
                    Debug.LogError("Error m_StoreController");
                    Debug.LogException(e);
                    product = null;
                }
        ...
        private void LogBuyFailFirebase(Product product, string failureReason)
        {
            var id = product.definition.storeSpecificId;
            if (id.Equals(_idSubscriptionMonthly))
            {
                FirebaseManager.instance.LogBuyFailIAP("monthly_plan", product.metadata.localizedPriceString, product.metadata.isoCurrencyCode, failureReason);
            }
            else if (id.Equals(_idSubscriptionAnnual))
            {
                FirebaseManager.instance.LogBuyFailIAP("annual_plan", product.metadata.localizedPriceString, product.metadata.isoCurrencyCode, failureReason);
            }
            else if (id.Equals(_idUnlockAll))
            {
                FirebaseManager.instance.LogBuyFailIAP("life_plan", product.metadata.localizedPriceString, product.metadata.isoCurrencyCode, failureReason);
            }
        }
        private void LogBuySuccessFirebase(Product product)
        {
            var id = product.definition.storeSpecificId;
            if (id.Equals(_idSubscriptionMonthly))
            {
                FirebaseManager.instance.LogBuySuccessIAP("monthly_plan", product.metadata.localizedPriceString, product.metadata.isoCurrencyCode);
            }
            else if (id.Equals(_idSubscriptionAnnual))
            {
                FirebaseManager.instance.LogBuySuccessIAP("annual_plan", product.metadata.localizedPriceString, product.metadata.isoCurrencyCode);
            }
            else if (id.Equals(_idUnlockAll))
            {
                FirebaseManager.instance.LogBuySuccessIAP("life_plan", product.metadata.localizedPriceString, product.metadata.isoCurrencyCode);
            }
        }
        private void LogBuySuccessFirebase(PurchaseEventArgs args)
        {
        ...
        public void OnPurchaseFailed(Product product, PurchaseFailureReason failureReason)
        {
            ...
            LogBuyFailFirebase(product, failureReason.ToString());
        ...
        public void OnPurchaseFailed(Product product, PurchaseFailureDescription failureDescription)
        {
            ...
            LogBuyFailFirebase(product, failureDescription.reason.ToString());
