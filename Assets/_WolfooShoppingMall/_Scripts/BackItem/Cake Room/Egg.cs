...
namespace _WolfooShoppingMall
{
    public class Egg : Food
    {
        ...
        public override void OnEndDrag(PointerEventData eventData)
        {
            ...
            EventDispatcher.Instance.Dispatch(new EventKey.OnEndDragBackItem { backitem = this, food = this, egg = this });
        }
        ...
        protected override void GetEndDragItem(EventKey.OnEndDragBackItem item)
        {
            ...
            if(!isTransformToBread && item.flour != null)
            {
                var distance = Vector2.Distance(item.flour.transform.position, transform.position);
                if(distance < 1)
                {
