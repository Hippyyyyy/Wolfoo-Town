...
namespace _WolfooShoppingMall
{
    ...
    public class CustomSnap : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        #region Fields
        private static CustomSnap ins;
        public static CustomSnap Ins => ins;
        private void Awake()
        {
        ...
        [SerializeField] CharacterPaletteController characterPaletteController;
        ...
        [SerializeField] private Transform paginationCheck = null;
        ...
        private bool ValidConfig
        {
            get
            {
                ...
                {
                    int numberOfToggles = paginationCheck.transform.childCount;
        ...
        public Transform PaginationCheck { get => paginationCheck; set => paginationCheck = value; }
        #endregion
        ...
        public void OnEndDrag(PointerEventData eventData)
        {
            ...
            {
                Panels[SelectedPanel].GetComponent<CharacterSlotView>().FirstClick();
                Panels[SelectedPanel].GetComponent<CharacterSlotView>().IsDragging = true;
                //
            });
        }
        ...
        public void Setup()
        {
            ...
            for (int i = 0; i < NumberOfPanels; i++)
            {
                ...
                if (movementType == MovementType.Fixed && useAutomaticLayout)
                {
                    ...
                    Panels[i].GetComponent<CharacterSlotView>().DefaultPos = Panels[i].anchoredPosition;
                }
            }
            ...
            // Pagination
            if (paginationCheck != null && NumberOfPanels != 0)
            {
                Toggles = paginationCheck.GetComponentsInChildren<ButtonScroll>();
                for (int i = 0; i < Toggles.Length; i++)
                {
                    int panelNumber = i;
                    Toggles[i].GetComponent<Button>().onClick.AddListener(() =>
                    {
                        if (paginationCheck != null)
                        {
        ...
        {
            characterPaletteController.GetCharacter(Panels[panelNumber].GetComponent<CharacterSlotView>().CurrentCharacter);
            CenteredPanel = panelNumber;
            SetUpdateEnabled(false);
            Panels[panelNumber].GetComponent<CharacterSlotView>().DeActiveFloop();
            scrollRect.enabled = false;
            float targetXCentered = 0f;
            foreach (var item in Panels)
            {
                item.GetComponent<CharacterSlotView>().EnableButton(false);
            }
            characterPaletteController.Show();
            AdsManager.Instance.HideBanner();
            //
            DOVirtual.DelayedCall(0.2f, ()=> {
                Panels[panelNumber].transform.SetParent(parent);
                Panels[panelNumber].DOAnchorPosX(targetXCentered, 0.7f).OnComplete(() =>
                {
                    ...
                    characterPaletteController.InitButtonRandomItemCharacter();
                    //Content.anchoredPosition = new Vector2(-Panels[panelNumber].anchoredPosition.x, Content.anchoredPosition.y);
                });
                float targetXOffset = 8 * Viewport.rect.width;
                for (int i = 0; i < Panels.Length; i++)
                {
                    if (i != panelNumber)
                    {
                        float targetX = (i < panelNumber) ? -30 : 30;
                        Panels[i].DOMoveX(targetX, 0.7f);
                    }
                }
            });
        }
        ...
        public void ShowPanel(int panelNumber)
        {
            ...
            DOVirtual.DelayedCall(0.2f, () =>
            {
                characterPaletteController.RemoveButtonRandomItemCharacter();
                characterPaletteController.Hide();
                btnBack.onClick.RemoveAllListeners();
                SetBackButtonToHome();
                Panels[panelNumber].transform.SetParent(Content); // Set parent back to Content
                Panels[panelNumber].transform.SetSiblingIndex(panelNumber);
                float targetXCentered = Panels[panelNumber].anchoredPosition.x;
                Panels[panelNumber].GetComponent<CharacterSlotView>().ShowFloop();
                Panels[panelNumber].DOAnchorPosX(Panels[panelNumber].GetComponent<CharacterSlotView>().DefaultPos.x, 1f).OnComplete(() =>
                {
                    ...
                    foreach (var item in Panels)
                    {
                        item.GetComponent<CharacterSlotView>().EnableButton(true);
                        item.GetComponent<CharacterSlotView>().IsFirstClick = true;
                    }
                    Panels[panelNumber].GetComponent<CharacterSlotView>().DeActiveFirstClick();
                });

                float targetXOffset = 10 * Viewport.rect.width;
                for (int i = 0; i < Panels.Length; i++)
                {
                    if (i != panelNumber)
                    {
                        float targetX = (i < panelNumber) ? -targetXOffset : targetXOffset;
                        Panels[i].DOAnchorPosX(Panels[i].GetComponent<CharacterSlotView>().DefaultPos.x, 1f);
                        Panels[i].GetComponent<CharacterSlotView>().HideFloop();
                    }
                }
            });
        }
        ...
        private int GetNearestPanel()
        {
            ...
            float minDistance = distances[0]; // Start with the distance of the first panel
        ...
        private void AttachButtonEvents()
        {
            ...
            {
                int panelIndex = i;
                Button panelButton = Panels[i].gameObject.GetComponent<Button>();

                if (panelButton == null)
                {

                    panelButton = Panels[i].gameObject.AddComponent<Button>();
                }
                panelButton.onClick.AddListener(() => OnPanelClicked(panelIndex));
            }
            SetBackButtonToHome();
        }
