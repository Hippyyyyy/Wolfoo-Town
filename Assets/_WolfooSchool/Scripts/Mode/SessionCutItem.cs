...
namespace _WolfooSchool
{
    public class SessionCutItem : Panel
    {
        ...
        private void OnDestroy()
        {
            ...
            if (_jumpTween != null) _jumpTween?.Kill();
            if (loopTween != null) loopTween?.Kill();
            if (punchTween != null) punchTween?.Kill();
            if (punchTween1 != null) punchTween1?.Kill();
            if (punchTween2 != null) punchTween2?.Kill();
        }
        /// <summary>
        /// Get Tutorial
        /// </summary>
        void GetTut()
        {
            _scaleTween?.Kill();
            _tween?.Kill();
            _tween = dotImg.DOFillAmount(1, 1.5f).OnComplete(() =>
            {
                if (isClickPanel)
                {
                ...
                highlightPanel.transform.localPosition = Vector3.zero;
                _scaleTween = highlightPanel.transform.DOScale(23, 1).OnComplete(() =>
                {
                    // Tutorial
                    if (isClickPanel)
                    {
        ...
        private void GetClickTutorial()
        {
            ...
            _scaleTween?.Kill();
            _tween?.Kill();
            _scaleTween = highlightPanel.transform.DOScale(100, 0.5f).OnComplete(() =>
              {
                  highlightPanel.gameObject.SetActive(false);
                  dragFollow.DragObject.gameObject.SetActive(true);
                  _tween = dragFollow.DragObject.transform
        ...
        private void OnEndSession()
        {
            ...
            _jumpTween?.Kill();
            _jumpTween = blockBtn.transform.DOJump(Vector3.zero, 1, 1, 2f)
            .OnComplete(() =>
            {
        ...
        private void OnOpenBlockShape()
        {
            ...
            _tween = blockBtn.transform.DOScale(1, 1).OnComplete(() =>
            {
        ...
        private void OnCompleteCutBox()
        {
            ...
            mainPaper.DOFade(0, time).OnComplete(() =>
            {
                ...
                .OnComplete(() =>
                {
                    ...
                    .OnComplete(() =>
                    {
                        TutorialManager.Instance.StartPointer(sliceBtn.transform.position, Gesture.Click);
