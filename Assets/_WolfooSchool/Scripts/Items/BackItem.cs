...
namespace _WolfooSchool
{
    public class BackItem : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IPointerClickHandler
    {
        ...
        protected ItemsDataSO data;
        private GameObject myMask;
        private bool isRegisterLoadEvent;
        ...
        {
            isRegisterLoadEvent = true;
            EventDispatcher.Instance.RegisterListener<EventKey.OnLoadDataCompleted>(InitData);
            //      var maskRotate = GameManager.instance.MaskRotatePb;
            //   myMask = Instantiate(maskRotate, transform);
            //   myMask.transform.SetAsFirstSibling();
        ...
        protected virtual void OnDestroy()
        {
            ...
            tweenJump?.Kill();
            delayTween?.Kill();
            if (isRegisterLoadEvent) EventDispatcher.Instance.RemoveListener<EventKey.OnLoadDataCompleted>(InitData);
        }
        void Dancing()
        {
        ...
        public void MoveToGround(bool isForceMove = false)
        {
            if (!canMoveToGround && !isForceMove) return;
        ...
        public void JumpOutSide(Vector3 endPos)
        {
            ...
            tweenJump = transform.DOJump(endPos, 1, 1, 0.5f).OnComplete(() =>
            {
            });
        }
